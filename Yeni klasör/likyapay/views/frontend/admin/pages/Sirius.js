...
const CycleGraph = ({ group }) => {
    if (!group || !group.node_names) return null;

    const count = group.node_names.length;
    const radius = 120;
    const center = 150;
    const size = 300;

    // Calculate node positions
    const nodes = group.node_names.map((name, i) => {
        const angle = (i * 2 * Math.PI) / count - (Math.PI / 2); // Start from top
        return {
            x: center + radius * Math.cos(angle),
            y: center + radius * Math.sin(angle),
            name: name,
            amount: group.details && group.details[i] ? parseFloat(group.details[i].amount) : 0
        };
    });

    return (
        <div className="flex justify-center bg-slate-50 rounded-xl p-4 border border-slate-100 relative overflow-hidden">
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
                ...
                {nodes.map((node, i) => {
                    ...
                    // Quadratic curve for smoother loop
                    ...
                    return (
                        <g key={`edge-${i}`}>
                            ...
                            <rect x={(node.x + nextNode.x) / 2 - 30} y={(node.y + nextNode.y) / 2 - 10} width="60" height="20" rx="4" fill="white" stroke="#e2e8f0" />
                            <text
                                x={(node.x + nextNode.x) / 2}
                                y={(node.y + nextNode.y) / 2 + 4}
                                textAnchor="middle"
                                fontSize="10"
                                fill="#64748b"
                                fontWeight="bold"
                ...
                {nodes.map((node, i) => (
                    <g key={`node-${i}`}>
                        <circle cx={node.x} cy={node.y} r="24" fill="white" stroke="#6366f1" strokeWidth="2" />
                        <text x={node.x} y={node.y} dy="5" textAnchor="middle" fill="#6366f1" fontSize="12" fontWeight="bold">
                            {i + 1}
                        </text>
                        <text x={node.x} y={node.y + 40} textAnchor="middle" fill="#1e293b" fontSize="12" fontWeight="bold">
            ...
            {/* Legend/Info */}
            <div className="absolute top-2 left-2 text-[10px] text-slate-400 bg-white/80 p-2 rounded shadow-sm border border-slate-100">
                <div className="flex items-center gap-1 mb-1"><div className="w-2 h-2 rounded-full bg-indigo-500"></div> Firma</div>
                <div className="flex items-center gap-1"><div className="w-4 h-[2px] bg-slate-300 border-t border-dashed"></div> Borç İlişkisi</div>
            </div>
        </div>
    );
};
    ...
    return (
        <div className="animate-fade-in space-y-8">
            ...
            <div className="flex justify-between items-end">
                ...
                >
                    <i className="fas fa-play"></i> Sirius
                </button>
            </div>
                    ...
                    <div className="divide-y divide-slate-50">
                        {groups.filter(g => activeTab === 'active' ? g.status !== 'completed' : g.status === 'completed').map(group => (
                            <div key={group.id} className="p-6 hover:bg-slate-50 transition group">
                                ...
                                {/* REPLACED: Chain Visualization with CycleGraph */}
                                <div className="mt-4">
                                    <CycleGraph group={group} />
                                </div>
                                ...
                                <div className="w-full mt-4 bg-slate-50 border border-slate-200 rounded-xl p-4">
                                    ...
                                    <div className="space-y-2">
                                        {selectedGroup.node_names.map((name, i) => {
                                            ...
                                            // Kullanıcı Talebi: 
                                            // 5'li (A-B-C-D-E) -> D ve E Mahsuplaşma, diğerleri Temlik.
                                            // 3'lü (A-B-C) -> B ve C Mahsuplaşma, A Temlik.
                                            // Genel Kural: Listenin son 2 elemanı Mahsuplaşma yapar.
