    ...
    if ($method === 'GET') {
        ...
        if ($userTax && $relatedTax) {
             ...
             while ($row = $cycleStmt->fetch(PDO::FETCH_ASSOC)) {
                 ...
                 if (is_array($nodes) && in_array($userTax, $nodes) && in_array($relatedTax, $nodes)) {
                     // Check if they are actually adjacent or relevant in the cycle?
                     // For strict data integrity, if both are in the loop, any debt change is risky.
                     // The user said: "döngüdeki o alacak vereceğini revize etmemeli"
                     throw new Exception("Bu borç/alacak kaydı aktif bir Sirius Döngüsü (#{$row['id']}) içinde işlem gördüğü için düzenlenemez.");
                 }
             }
        }
        // -------------------------
        ...
        if ($userTax && $relatedTax) {
             ...
             while ($row = $cycleStmt->fetch(PDO::FETCH_ASSOC)) {
                 $nodes = json_decode($row['nodes'], true);
                 if (is_array($nodes) && in_array($userTax, $nodes) && in_array($relatedTax, $nodes)) {
                     throw new Exception("Bu borç/alacak kaydı aktif bir Sirius Döngüsü (#{$row['id']}) içinde işlem gördüğü için silinemez.");
                 }
             }
        }
        // -------------------------
