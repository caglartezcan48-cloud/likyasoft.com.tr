...
try {
    ...
    $sector = $data->sector ?? '';
    $kep_address = $data->kepAddress ?? ''; // NEW

    // Let's use safer insert matching known schema from setup_accounting (users table update)
    $query = "INSERT INTO users (name, email, password, role, tax_id, status, phone, user_type, permissions, username, sector, kep_address, created_at) 
              VALUES (:name, :email, :password, :role, :tax_id, :status, :phone, :user_type, :permissions, :username, :sector, :kep_address, NOW())";
    ...
    $stmt->bindParam(":kep_address", $kep_address);
