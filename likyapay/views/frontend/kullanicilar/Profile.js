...
window.Kullanicilar.Profile = ({ user, onClose }) => {
    // Form State
    ...
    const canEditIban = user && ['admin', 'supplier', 'employee', 'personnel', 'worker', 'accountant'].includes(user.role);
    ...
    const handleSaveProfile = async () => {
        ...
        const missing = mandatoryFields.filter(f => !formData[f.key] || formData[f.key].trim() === '');
        if (missing.length > 0) {
            return alert(`Lütfen şu alanları doldurunuz: ${missing.map(m => m.label).join(', ')}`);
        }

        if (canEditIban && formData.iban && !window.Validators.isValidIBAN(formData.iban)) {
            return alert("Lütfen geçerli bir TR IBAN giriniz (TR + 24 hane).");
        }
        if (formData.tax_office && formData.tax_office.length < 3) {
            return alert("Vergi Dairesi en az 3 karakter olmalıdır.");
        }
                ...
                {activeTab === 'company' && (
                    <div className="space-y-6 animate-fade-in">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-700">İlçe</label>
                                <input
                                    type="text"
                            ...
                            {/* IBAN - Restricted to Admin, Supplier, Employee */}
                            {['admin', 'supplier', 'employee', 'personnel', 'worker', 'accountant'].includes(user?.role) && (
                                <div className="space-y-2 md:col-span-2">
                                    <label className="text-sm font-bold text-slate-700">IBAN <span className="text-red-500 text-xs">(Alacak tahsilatı için)</span></label>
                                    <input
                                        type="text"
                                        value={formData.iban}
                                        onChange={(e) => {
                                            ...
                                            // Use validator utility if key exists, else manual regex
                                            if (window.Validators) {
                                                // Strip non-alphanumeric first
                                                val = val.replace(/[^A-Z0-9]/g, '');
                                                if (!val.startsWith('TR')) {
                                                    // Should we force TR? Let's just allow typing but format
                                                }
                                                // Limit to 26 chars (ignoring spaces) -> actually 26 + spaces
                                                if (val.length > 26) val = val.slice(0, 26);

                                                setFormData({ ...formData, iban: window.Validators.formatIBAN(val) });
                                            } else {
                                                setFormData({ ...formData, iban: val });
                                            }
                                        }}
                                        className={`w-full px-4 py-2 rounded-lg border focus:ring-2 outline-none transition font-mono ${formData.iban && window.Validators && !window.Validators.isValidIBAN(formData.iban)
                                            ? 'border-red-300 focus:ring-red-200 bg-red-50'
                                            : 'border-slate-300 focus:ring-brand-500'
                                            }`}
                                        placeholder="TR..."
                                    />
                                    {formData.iban && window.Validators && !window.Validators.isValidIBAN(formData.iban) && (
                                        <p className="text-xs text-red-500">Geçersiz IBAN formatı (TR + 24 hane)</p>
                                    )}
                                </div>
                            )}
