...
window.Admin.Dashboard = ({ onLogout }) => {
        ...
        const loadData = async () => {
            try {
                ...
                const [userRes, txRes, sysRes] = await Promise.all([
                    fetch('../data/api/list_users.php'), // Switched to robust API
                ...
                if (userData.success && userData.users) {
                    // list_users.php already returns mapped data (title, taxNo, status='pending'/'verified')
    ...
    const renderPage = () => {
        ...
        switch (view) {
            ...
            case 'sirius_archive':
                return window.Admin.Pages.SiriusArchive ? <window.Admin.Pages.SiriusArchive /> : <div>Sirius Arşiv Modülü Yüklenemedi</div>;
            case 'admin_users':
            ...
            case 'pending_invoices': // NEW
                return window.Admin.Pages.PendingInvoices ? <window.Admin.Pages.PendingInvoices systemTransactions={systemTransactions} /> : <div>Fatura Ekranı Yüklenemedi</div>;
            case 'messages':
                return window.Admin.Pages.Messages ? <window.Admin.Pages.Messages /> : <div>Mesaj Modülü Yüklenemedi</div>;
