...
window.Admin.Pages.DashboardHome = ({ users, pendings, transactions }) => {
    ...
    const totalDebt = transactions
        .filter(t => t.type === 'debt' && t.status !== 'rejected' && t.status !== 'cancelled')
        .reduce((acc, t) => acc + parseFloat(t.amount || 0), 0);

    const totalCredit = transactions
        .filter(t => t.type === 'credit' && t.status !== 'rejected' && t.status !== 'cancelled')
        .reduce((acc, t) => acc + parseFloat(t.amount || 0), 0);
    ...
    const stats = [
        ...
        { id: 4, label: 'Sirius Geliri', value: new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY', maximumFractionDigits: 2 }).format(transactions.reduce((acc, t) => acc + parseFloat(t.commission || 0), 0)), icon: 'fa-coins', color: 'bg-amber-500', trend: '+' }
    ];
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex justify-between items-end">
                ...
                <div>
                    <button className="bg-brand-900 hover:bg-brand-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {stats.map(stat => (
                    <div key={stat.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition">
                        <div className="flex items-center justify-between mb-4">
                            <div className={`w-12 h-12 rounded-lg ${stat.color} bg-opacity-10 flex items-center justify-center text-xl ${stat.color.replace('bg-', 'text-')}`}>
