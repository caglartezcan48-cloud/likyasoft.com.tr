...
class SiriusEngine {
    ...
    public function __construct() {
        ...
        // Ensure sirius_cycles table exists and has all columns
        try {
            $this->db->exec("CREATE TABLE IF NOT EXISTS sirius_cycles (id INT AUTO_INCREMENT PRIMARY KEY) ENGINE=InnoDB;");
            
            $stmt = $this->db->query("DESCRIBE sirius_cycles");
            $columns = $stmt->fetchAll(PDO::FETCH_COLUMN);
            
            $required = [
                'cycle_code' => "VARCHAR(50) NULL",
                'cycle_hash' => "VARCHAR(100) NULL",
                'nodes' => "TEXT NULL",
                'details' => "TEXT NULL",
            ...
            foreach ($required as $col => $def) {
                if (!in_array($col, $columns)) {
                    $this->db->exec("ALTER TABLE sirius_cycles ADD COLUMN $col $def");
                }
            }

            // Fix for 'node_names' error if it exists manually
            if (in_array('node_names', $columns)) {
                $this->db->exec("ALTER TABLE sirius_cycles MODIFY COLUMN node_names TEXT NULL");
            }
    ...
    private function loadBlockedUsers() {
        $sql = "SELECT nodes FROM sirius_cycles WHERE status IN ('detected', 'approved', 'processing', 'payment_stage', 'legal_stage')";
    ...
    private function analyzeCycle($nodes) {
        ...
        if ($min_amount <= 0) return; // Prevent 0 or negative cycles
    ...
    private function saveCycle($cycle) {
        ...
        // Count ALL cycles (Global Sequence) to determine sequence number since day 1
        $countSql = "SELECT COUNT(*) FROM sirius_cycles";
        $stmtCount = $this->db->prepare($countSql);
        $stmtCount->execute();
        $globalCount = $stmtCount->fetchColumn();
        
        // Format: DDMMYY-GlobalCount (e.g. 050126-145)
        $sequence = str_pad($globalCount + 1, 2, "0", STR_PAD_LEFT);
        $cycleCode = $dateStr . "-" . $sequence;
