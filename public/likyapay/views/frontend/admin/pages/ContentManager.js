...
window.Admin.Pages.ContentManager = ({ showToast }) => {
    ...
    const [content, setContent] = React.useState({
        ...
        legal: {
            title: "Hukuki Altyapı ve Dayanaklar",
            section1: "Likya Pay; şirketler arasındaki ticari alacak ve borçların, mevcut Türk mevzuatı kapsamında, temlik (alacağın devri) ve mahsuplaşma (takas) yöntemleriyle bilanço içinde optimize edilmesini sağlayan bir finansal uzlaşma ve optimizasyon sistemidir. Likya Pay: Para toplamaz, kredi vermez, ödeme hizmeti sunmaz, bankacılık faaliyeti yürütmez.",
            section2: "A) Alacağın Devri (Temlik) - TBK m.183–194: Alacaklı, alacağını üçüncü kişiye devredebilir. B) Mahsuplaşma (Takas) - TBK m.139–145: Taraflar karşılıklı alacaklı ve borçlu ise borçlar sona erdirilebilir.",
            section3: "Likya Pay, klasik ikili mahsuplaşmadan farklı olarak en az 3 şirketten oluşan uzlaşma halkaları kurar. Her işlem belgelidir, izlenebilir ve denetlenebilir.",
            section4: "Likya Pay ödeme kuruluşu değildir, elektronik para ihraç etmez. Ticari alacak yönetimi ve optimizasyonu yapar.",
            section5: "Hizmet bedeli %3 + KDV'dir. Bu bedel finansman faizi değil, danışmanlık ve organizasyon hizmet bedelidir.",
            section6: "Üyelik Katılım Sözleşmesi, Alacak Temlik Sözleşmesi, Mahsup ve Uzlaşma Bildirimi, Döngü Mutabakat Tutanağı, Hizmet Bedeli Faturası.",
            section7: "Likya Pay modeli Türk Borçlar Kanunu’na uygundur, özel hukuk sözleşmelerine dayanır ve finansal regülasyon ihlali içermez."
        }
    });
    ...
    const tabs = [
        ...
        { id: 'legal', label: 'Hukuki Altyapı', icon: 'fa-gavel' },
        ...
        <div className="animate-fade-in space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 bg-white p-6 rounded-xl shadow-sm border border-gray-200 min-h-[400px]">
                    ...
                    {activeTab === 'legal' && (
                        <div className="space-y-6 animate-fade-in">
                            <h3 className="text-lg font-bold text-gray-800 border-b pb-2">Hukuki Altyapı ve Dayanaklar</h3>
                            <p className="text-sm text-gray-500 mb-4">Web sitesindeki "Yasal Çerçeve" modal içeriğini buradan güncelleyebilirsiniz.</p>

                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Başlık</label>
                                <input
                                    type="text"
                                    value={content.legal?.title || ''}
                                    onChange={(e) => handleChange('legal', 'title', e.target.value)}
                            ...
                            {[1, 2, 3, 4, 5, 6, 7].map(num => (
                                <div key={num}>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">{num}. Bölüm İçeriği</label>
                                    <textarea
                                        rows="3"
                                        value={content.legal?.[`section${num}`] || ''}
                                        onChange={(e) => handleChange('legal', `section${num}`, e.target.value)}
                                        className="w-full px-4 py-2 border border-gray-300 rounded-lg outline-none"
                                    ></textarea>
                                </div>
                            ))}
