...
try {
    ...
    $identifier = $data->email; // Can be email, username, or phone
    $password = $data->password;

    // Check email, username, name (company name), or phone
    $query = "SELECT id, name, username, email, password, role, tax_id, status, user_type, permissions 
              FROM users 
              WHERE email = :id OR username = :id OR name = :id OR phone = :id 
              LIMIT 0,1";
