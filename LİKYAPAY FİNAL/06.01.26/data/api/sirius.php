...
try {
    ...
    elseif ($action === 'download_invoice') {
        ...
        // 3. Find Transaction (Service Fee)
        // We look for a system transaction for this user related to this cycle
        $descSearch = "%Sirius Döngü #{$cycle['id']}%";
        $tStmt = $db->prepare("SELECT * FROM system_transactions WHERE entity_name = :name AND description LIKE :desc LIMIT 1");
        $tStmt->execute([':name' => $user['name'], ':desc' => $descSearch]);
        $tx = $tStmt->fetch(PDO::FETCH_ASSOC);

        if (!$tx) {
             // Fallback: Calculate what it SHOULD be if not found (e.g. not yet approved?)
             // But usually it's created as 'draft' in finalize_cycle.
             // If missing, maybe create mock or die.
             // Let's generate based on cycle data (safe fallback)
             $volume = (float)$cycle['total_volume'];
             ...
             // 3b. Try to find record in system_transactions (Kasa Defteri)
             // Logic: Match Name AND Description contains "Sirius Döngü #ID"
             $accStmt = $db->prepare("SELECT * FROM system_transactions 
                                      WHERE entity_name = :name 
                                      AND description LIKE :desc 
                                      AND type = 'income'
                                      LIMIT 1");
             $searchDesc = "%Sirius Döngü #{$cycle['id']}%";
             $accStmt->execute([':name' => $user['name'], ':desc' => $searchDesc]);
             $accTx = $accStmt->fetch(PDO::FETCH_ASSOC);

             if ($accTx) {
                 // Use Accounting Record
                 $baseAmount = (float)$accTx['amount']; 
                 // Note: system_transactions amount is usually fee + vat (Total).
                 // But wait, complete_cycle inserts $feePerNode = $volume * 0.03 * 1.2;
                 // So 'amount' is Total (Gross).
                 $totalAmount = $baseAmount;
                 $baseAmount = $totalAmount / 1.2;
                 $vatAmount = $totalAmount - $baseAmount;
                 $vatRate = 20;
                 $txDate = $accTx['date'];
                 $txId = "ACC-" . $accTx['id'];
             } else {
                 // Fallback: Calculate from Cycle Volume
        ...
        // 4. Generate HTML (Template copied from system_accounting)
        ...
        $date = date('d.m.Y', strtotime($txDate));
