...
window.Kullanicilar.DashboardHome = ({ setPage, summary, transactions, user }) => {
    ...
    // Routing for Blue Collar Employees (Direct to Salary Dashboard)
    if (user.user_type === 'employee' && user.employee_role === 'blue_collar') {
        const SalaryDashboard = window.Kullanicilar.SalaryDashboard;
        return <SalaryDashboard user={user} />;
    }
