...
window.Kullanicilar.Panel = ({ onLogout, user }) => {
    ...
    React.useEffect(() => {
        if (user) {
            ...
            const missing = required.filter(f => !user[f.key] || user[f.key].toString().trim() === '').map(f => f.label);
    ...
    const summary = React.useMemo(() => {
        ...
        const debtCents = transactions
            ...
            .reduce((acc, t) => acc + Math.round((parseFloat(t.amount) || 0) * 100), 0);
        ...
        return {
            ...
            balance: (creditCents - debtCents) / 100,
    ...
    const renderContent = () => {
        switch (activePage) {
            ...
            case 'sirius':
                return <window.Kullanicilar.Sirius user={user} isProfileComplete={isProfileComplete} setPage={setActivePage} />;
            case 'archive':
                return <window.Kullanicilar.Archive />;
            case 'profile':
                return <window.Kullanicilar.Profile user={user} onClose={() => setActivePage('dashboard')} />;
            case 'messages':
                return <window.Kullanicilar.Messages user={user} />;
            case 'my_companies':
                return <window.Kullanicilar.MyCompanies />;
            default:
        ...
        <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
            ...
            <div className="flex-1 flex flex-col min-w-0 transition-all duration-300">
                ...
                <header className="md:hidden bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-30">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-600 hover:text-brand-600 hover:bg-brand-50 transition relative"
                        >
                            <i className="fas fa-bell"></i>
                            {/* Optional: <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></span> */}
                        </button>
                ...
                <header className="hidden md:flex bg-white/80 backdrop-blur border-b border-slate-200 px-8 py-4 justify-between items-center sticky top-0 z-30">
                    <h2 className="text-xl font-bold text-slate-800 capitalize">
                        {activePage === 'dashboard' ? 'Genel Bakış' :
                            activePage === 'invoices' ? 'Borç ve Alacaklar' :
                                activePage === 'messages' ? 'Mesajlar & Bildirimler' : activePage}
                    </h2>
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full text-xs text-slate-600 font-medium">
                        ...
                        <button
                            onClick={() => setActivePage('messages')}
                            className="w-9 h-9 rounded-full bg-white border border-slate-200 text-slate-500 hover:text-brand-600 hover:border-brand-200 hover:bg-brand-50 transition flex items-center justify-center shadow-sm relative group"
                            title="Bildirimler ve Mesajlar"
                        >
                            <i className="fas fa-bell text-lg group-hover:animate-swing"></i>
                            {/* Badge demo */}
                        ...
                        <button onClick={onLogout} className="text-slate-500 hover:text-red-600 flex items-center gap-2 transition text-sm font-medium hover:bg-red-50 px-3 py-1.5 rounded-lg">
                ...
                <main className="flex-1 p-4 lg:p-8 overflow-y-auto">
                    ...
                    {!isProfileComplete && activePage !== 'profile' && (
                        <div className="mb-6 bg-orange-50 border-l-4 border-orange-500 p-4 rounded-r shadow-sm flex items-start justify-between animate-fade-in">
                            <div className="flex gap-3">
                                <div className="mt-1 text-orange-500">
                                    <i className="fas fa-exclamation-triangle"></i>
                                </div>
                                <div>
                                    <h4 className="font-bold text-orange-800">Profil Bilgileriniz Eksik</h4>
                                    <p className="text-sm text-orange-700 mt-1">
                                        Sirius döngülerine katılabilmek için şirket bilgilerinizi tamamlamanız gerekmektedir.
                                        <br />
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-orange-100 text-orange-700 font-bold rounded hover:bg-orange-200 transition text-sm whitespace-nowrap"
                            >
                                Tamamla <i className="fas fa-arrow-right ml-1"></i>
                            </button>
                        </div>
                    )}
